...
Our distribution includes an `executable reference implementation of the language <formal-semantics_>`_ written using the `Coq proof assistant <https://rocq-prover.org/>`_, machine-checked proofs ensuring that |koika|'s semantics are compatible with `one-rule-at-a-time execution <oraat_>`_, and a `formally-verified compiler <compiler-verification_>`_ that generates circuits guaranteed to correctly implement your designs.
...
Details about |koika|\ 's design and implementation can be found in our `research paper <https://dl.acm.org/doi/pdf/10.1145/3385412.3385965>`_.
...
We wrote `a paper about cuttlesim <https://dl.acm.org/doi/pdf/10.1145/3445814.3446720>`__.
...
BEAK fuzzing support
===============

This fork also contains BEAK, an AFL++-based fuzzing extension for |koika| designs compiled with Cuttlesim. BEAK generates a fuzzing harness around the Cuttlesim-generated C++ model, maps fuzzer-generated bytes to selected initial state and input channels, and reports assertion violations as crashes.

BEAK is intended as an early testing step for |koika| designs before deeper verification. It supports in-flight and after-execution assertions, crash decoding, and differential fuzzing experiments for the RV32I core.

The main harness generation code is in `ocaml/backends/harness.ml`. Some supporting changes were also applied to `ocaml/backends/cpp.ml`.

`AFL++ <https://github.com/AFLplusplus/AFLplusplus>`_ must be installed and available on the ``PATH`` to run fuzzing experiments.

The `beak/evaluation` directory contains the zipped evaluation data used in the report. It also contains two helper scripts:

* `run_tool.sh`: helper script for running fuzzing experiments. Run `./run_tool.sh --help` to see the available options, including harness mode and optimization flags.
* `setup_new_eval.sh`: helper script for extracting the files needed to create a new fuzzing folder for use with `run_tool.sh`.

The harness files are normally generated by running `make all`.
...
In the AFL++ crashes directory, two additional files should be generated for each crashing input:

* `*_decoded`: dump of the initial and final state of the crashing run.
* `*_decoded_ruletrace`: list of rules triggered in each cycle.

RISC-V differential fuzzing setup
~~~~~~~~~~~~

For the RISC-V setup, see `beak/evaluation/riscv_eval.zip`, in particular `riscv_eval/data/riscv_diff_eval`.
